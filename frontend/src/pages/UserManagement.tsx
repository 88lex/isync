...
const UserManagement = () => {
    ...
    const selectAllUsers = (visibleUsers: any[]) => {
        if (selectedUsers.size === visibleUsers.length && visibleUsers.length > 0) {
            ...
            setSelectedUsers(new Set(visibleUsers.map(u => u.primaryEmail)));
        }
    };
    ...
    const runBulkOp = async (action: BulkOpRequest['action']) => {
        ...
        const emailToDomain = new Map(users.map(u => [u.primaryEmail, u._sourceDomain]));
    ...
    const filteredUsers = users.filter(u => {
        const searchMatch = !filter || 
            u.primaryEmail.toLowerCase().includes(filter.toLowerCase()) || 
                            ...
                            <tbody className="divide-y divide-zinc-800/50">
                                {filteredUsers.length === 0 ? (
                                    ...
                                    filteredUsers.map((u, i) => {
                                        const isProtected = config.protected_users?.includes(u.primaryEmail);
                                        return (
                                            <tr key={i} className={`group hover:bg-zinc-800/50 transition ${selectedUsers.has(u.primaryEmail) ? 'bg-indigo-900/10' : ''}`}>
                                                <td className="p-4">
                                                    <input 
                                                        type="checkbox"
                                                        className="rounded border-zinc-700 bg-zinc-800"
                                                        checked={selectedUsers.has(u.primaryEmail)}
                                                        onChange={() => toggleUser(u.primaryEmail)}
                                                    />
                                                </td>
                                                <td className="p-4">
                                                    <div 
                                                        className="font-medium text-zinc-200 flex items-center gap-2 cursor-pointer hover:text-white transition"
                                                        onClick={() => copyText(u.name?.fullName || u.primaryEmail.split('@')[0])}
                                                        title="Click to copy name"
                                                    >
                                                        {u.name?.fullName || u.primaryEmail.split('@')[0]}
                                                        {isProtected && (
                                                    ...
                                                    <div 
                                                        ...
                                                        onClick={() => copyText(u.primaryEmail)}
