...
IF %ERRORLEVEL% EQU 0 (
    ...
    wt -w 0 new-tab --title "ISync" pwsh -NoExit -Command "wsl -d Ubuntu -e bash -c 'cd /opt/isync && ./run_isync.sh'"
    EXIT /B 0
)
...
IF %ERRORLEVEL% EQU 0 (
    ...
    start "ISync" pwsh -NoExit -Command "wsl -d Ubuntu -e bash -c 'cd /opt/isync && ./run_isync.sh'"
    EXIT /B 0
)
...
IF %ERRORLEVEL% EQU 0 (
    ...
    start "ISync" powershell -NoExit -Command "wsl -d Ubuntu -e bash -c 'cd /opt/isync && ./run_isync.sh'"
    EXIT /B 0
)
...
wsl -d Ubuntu -e bash -c "cd /opt/isync && ./run_isync.sh"
